    ...
    private void printAccidentDetail(Accident accident) throws IOException {
        while(true){
            ...
            if (choice.equals("0")) {
                ...
                isSuccess = serviceContainer.getCompensateService().examineCompensation(accident, contractCompensation, AccidentStatus.Compensate);
                if(isSuccess) {System.out.println("보상이 완료되었습니다."); return;}
                else {System.out.println("보상이 실패하였습니다.");}
            } else if(choice.equals("2")){
                isSuccess = serviceContainer.getCompensateService().examineCompensation(accident, contractCompensation, AccidentStatus.RefuseCompensate);
